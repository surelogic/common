...
public class SendServiceMessageWizard extends Wizard {
   ...
   *          Windows).
   */
  public static void openProblemReport(String product, String imageSymbolicName) {
   ...
   *          Windows).
   */
  public static void openProblemReport(Shell shell, String product, String imageSymbolicName) {
    ProblemReportMessage prm = new ProblemReportMessage(JDTUtility.getEclipseLogFile());
    if (product == null)
      product = "UNKNOWN";
  ...
  /**
   * Used to open the Send Tip for Improvement dialog using the shell being used
   * by the active workbench window.
   * 
   * @param product
  ...
  /**
   * Used to open the Send Tip for Improvement dialog using the shell being used
   * by the active workbench window. The tip message may be populated with
   * information.
   * 
   * @param product
   *          the SureLogic tool this tip for improvement is about.
   ...
   *           if tip is {@code null}.
   */
  public static void openTip(String product, String imageSymbolicName, TipMessage tip) {
   ...
   * 
   * @param shell
   *          a shell, or {@code null} to use the shell being used by the active
   *          workbench window.
   * @param product
   *          the SureLogic tool this tip for improvement is about.
   * @param imageSymbolicName
   ...
   *           if tip is {@code null}.
   */
  public static void openTip(Shell shell, String product, String imageSymbolicName, Message tip) {
    if (tip == null)
   ...
   * 
   * @param product
   *          the SureLogic tool this crash report is about. Must be non-null.
   *          This is known to be JSure, however, the client may include a
   *          version as well as a name.
   * @param status
   *          the status message about the scan crash. Must be non-null.
   * @param scanLog
   *          the scan log file. Must be non-null.
   */
  public static void openJSureScanCrashReport(String product, SLStatus status, File scanLog) {
   ...
   * @param product
   *          the SureLogic tool this crash report is about. Must be non-null.
   *          This is known to be Sierra, however, the client may include a
   *          version as well as a name.
   * @param status
   *          the status message about the scan crash. Must be non-null.
   * @param scanLog
   *          the scan log file. Must be non-null.
   */
  public static void openSierraScanCrashReport(String product, SLStatus status, File scanLog) {
  ...
  private static void openHelper(Shell shell, Message message, final String imageSymbolicName) {
    ...
    final WizardDialog dialog = new WizardDialog(shell, wizard) {
      ...
      protected void setShellStyle(int newShellStyle) {
        ...
        super.setShellStyle(SWT.CLOSE | SWT.MAX | SWT.TITLE | SWT.BORDER | SWT.MODELESS | SWT.RESIZE | getDefaultOrientation());
      }
  ...
  SendServiceMessageCollectInformationPage f_collect;
  SendServiceMessagePreviewPage f_preview;
  ...
  public boolean performFinish() {
    ...
    final SLJob job = ServiceUtility.sendToSureLogic(msg, new Runnable() {
      @Override
      public void run() {
        BalloonUtility.showMessage(I18N.msg(f_data.propPfx() + "sent.title"), I18N.msg(f_data.propPfx() + "sent.message"));
      }
    });
    final SLJobWrapperRunnableWithProgress rwp = new SLJobWrapperRunnableWithProgress(job);
    try {
      getContainer().run(true, false, rwp);
    } catch (InvocationTargetException e) {
      ErrorDialogUtility.open(null, null, SLEclipseStatusUtility.convert(rwp.getResultAsSLStatus()));
      return false;
