...
public final class SLPrintingUtility {
  ...
  /**
   * Submits a UI job to print the passed text. This job will prompt the use for
   * the correct printer to print with and gives the user a chance to cancel the
   * printing.
   * 
   * @param title
   *          the title of the document, {@code null} if you don't want a title
   *          to be printed.
   * @param text
   ...
   *          otherwise.
   */
  static public void printText(final String title, final String text, final boolean printPageNumbers) {
    final UIJob job = new SLUIJob() {
      @Override
      public IStatus runInUIThread(IProgressMonitor monitor) {
        if (!isPrintingAvailable()) {
          // Can't print or no printers are installed
          MessageDialog.openInformation(EclipseUIUtility.getShell(), "Cannot Print", "Printing from Eclipse is unavailable.");
          return Status.OK_STATUS;
        }
        final PrintDialog printDialog = new PrintDialog(EclipseUIUtility.getShell());
        ...
        final Job printJob = new Job("Printing SureLogic Report") {
          ...
          protected IStatus run(IProgressMonitor monitor) {
            monitor.beginTask("Printing SureLogic Report", IProgressMonitor.UNKNOWN);
            try {
   ...
   *           if the specified printer data does not represent a valid printer.
   */
  static Printer getPrinter(final PrinterData data) {
    /**
  ...
  static void print(Printer printer, String title, String text, boolean printPageNumbers) {
    Config c = new Config();
    c.printer = printer;
    c.title = title;
    c.textToPrint = text + " ";
  ...
  private static Font getPrinterFont(Printer printer) {
    String[] names = { "Consolas", "Terminal", "Monaco", "Mono", "Anonymous Pro", "Courier New", "Courier" };
    /*
  ...
  private static void printText(Config c) {
    ...
    while (c.index < c.end) {
      ...
      if (ch != 0) {
        if (ch == 0x0a || ch == 0x0d) {
          if (ch == 0x0d && c.index < c.end && c.textToPrint.charAt(c.index) == 0x0a) {
            /*
  ...
  private static void decoratePage(Config c) {
    ...
    c.gc.setLineWidth(5);
    c.gc.drawRectangle(c.clientArea.x + padBox, c.clientArea.y + padBox, c.clientArea.width - padBox * 2, c.clientArea.height
        - padBox * 2);
