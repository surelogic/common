...
public final class JavaProjectSelectionDialog extends Dialog {
  ...
  public static class Configuration {
    ...
    public Configuration(final String label, final String shellTitle, final Image shellImage,
        final List<IJavaProject> initiallySelectedJavaProjects, final String alwaysChooseFromDialogPreferenceConstant,
   ...
   * 
   * @param config
   *          the dialog configuration. This contains information about what to
   *          display and what preferences to access.
   * @return
   */
  public static List<IJavaProject> getProjects(final Configuration config) {
     ...
     * there is no selection in the Package Explorer) or the user always wants
     * to choose from a dialog then we show the project selection dialog.
     */
    final boolean alwaysChooseFromDialog;
    if (config.f_alwaysChooseFromDialogPreferenceConstant != null) {
      alwaysChooseFromDialog = EclipseUtility.getBooleanPreference(config.f_alwaysChooseFromDialogPreferenceConstant);
    } else {
      alwaysChooseFromDialog = true;
    }
    if (config.f_initiallySelectedJavaProjects.isEmpty() || alwaysChooseFromDialog) {

      final List<IJavaProject> openJavaProjects = JDTUtility.getJavaProjects();
      ...
      if (openJavaProjects.isEmpty()) {
        final UIJob job = new SLUIJob() {
          ...
          public IStatus runInUIThread(final IProgressMonitor monitor) {
            ...
            final String msg = I18N.msg("common.dialog.noJavaProjectsOpen");
            final MessageDialog dialog = new MessageDialog(shell, "No Projects Open", config.f_shellImage, msg,
        ...
        /*
         * If no projects are selected then use the last set of projects scanned
         * (if any). Hopefully, this helps the tool user.
         */
        if (config.f_initiallySelectedJavaProjects.isEmpty()) {
          if (config.f_prvUsrSelProjectListPreferenceConstant != null) {
            List<String> prevSel = EclipseUtility.getStringListPreference(config.f_prvUsrSelProjectListPreferenceConstant);
            for (String projectName : prevSel) {
              IJavaProject javaProj = JDTUtility.getJavaProject(projectName);
              if (javaProj != null)
                config.f_initiallySelectedJavaProjects.add(javaProj);
            }
          }
        }
        ...
        final UIJob job = new SLUIJob() {
          ...
          public IStatus runInUIThread(final IProgressMonitor monitor) {
            final Shell shell = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell();
            final JavaProjectSelectionDialog dialog = new JavaProjectSelectionDialog(shell, config, openJavaProjects,
                mutableProjectList);
        ...
        if (status == Status.CANCEL_STATUS) {
          ...
          if (config.f_prvUsrSelProjectListPreferenceConstant != null) {
            ...
            EclipseUtility.setStringListPreference(config.f_prvUsrSelProjectListPreferenceConstant, usrSel);
          }
  ...
  protected Control createDialogArea(final Composite parent) {
    ...
    for (final IJavaProject jp : f_openJavaProjects) {
      ...
      item.setImage(SLImages.getImageForProject(jp));
      item.setData(jp);
    ...
    allNonePanel.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false));
  ...
  private final void setOKState() {
    ...
    if (f_projectTable != null && !f_projectTable.isDisposed()) {
       ...
       */
      getButton(IDialogConstants.OK_ID).setEnabled(!f_selectedProjects.isEmpty());
    }
  }
