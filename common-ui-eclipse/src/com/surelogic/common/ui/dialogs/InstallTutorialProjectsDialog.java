...
public class InstallTutorialProjectsDialog extends TitleAreaDialog {
   ...
   * @param projects
   *          the URL locations of each project to be added. The project should
   *          be in the form of <tt>Name.zip</tt>, where <tt>Name</tt> is the
   *          name of the eclipse project to be added. Typically these are
   *          created with a call to {@link ClassLoader#getResource(String)}.
   */
  public static void open(final Shell shell, final String logo, final String href, final URL... projects) {
    if (projects.length == 0)
      throw new IllegalArgumentException(I18N.err(173));

    final InstallTutorialProjectsDialog dialog = new InstallTutorialProjectsDialog(shell, logo, href, projects);
    dialog.open();
  }
   ...
   *          created with a call to {@link ClassLoader#getResource(String)}.
   */
  public static void open(final Shell shell, final String logo, final String href, final List<URL> projects) {
    open(shell, logo, href, projects.toArray(new URL[projects.size()]));
  }

  public InstallTutorialProjectsDialog(final Shell parentShell, final String logo, final String href, final URL[] projects) {
    super(parentShell);
  ...
  protected Control createDialogArea(final Composite parent) {
    ...
    projectTree.setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT, true, false));
    ...
    showHelp.setLayoutData(new GridData(SWT.DEFAULT, SWT.DEFAULT, false, false));
    showHelp.setSelection(true);

    setTitle(I18N.msg("common.tutorial.dialog.title"));
    setMessage(I18N.msg("common.tutorial.dialog.info"), IMessageProvider.INFORMATION);
      ...
      public SLStatus run(SLProgressMonitor monitor) {
        ...
        try {
          for (final URL project : projectList) {
            final IProject p = EclipseUtility.unzipToWorkspace(project);
            String workingProjectName = project.getPath();
            if (workingProjectName.length() > 5) {
              workingProjectName = workingProjectName.substring(5);
            }
            if (workingProjectName.length() > 5) {
              workingProjectName = workingProjectName.substring(0, workingProjectName.length() - 4);
            }
          ...
          final int errno = 174;
          return SLStatus.createErrorStatus(errno, I18N.err(errno), e);
        } finally {
          monitor.done();
        }
