...
public final class TableExportDialog extends Dialog {
  ...
  final ExportTableDataSource f_source;
  final String f_dialogTitle;
  ...
  public TableExportDialog(Shell shell, String dialogTitle, ExportTableDataSource source, String key) {
    super(shell);
    assert source != null;
    f_source = source;
    if (dialogTitle == null) {
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    directions.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1));
    directions.setText("Select the desired export" + " format and the destination filename");
    ...
    f_exportFilenameText.setText(System.getProperty("user.home") + System.getProperty("file.separator") + "export.csv");
    ...
    browseButton.addListener(SWT.Selection, new Listener() {
      ...
      public void handleEvent(Event event) {
        if (fd == null) {
          ...
          fd.setFilterNames(new String[] { "CSV Files (*.csv)", "HTML Files (*.html)", "All Files (*.*)" });
        }
  ...
  void changeFileExtension(String from, String to) {
