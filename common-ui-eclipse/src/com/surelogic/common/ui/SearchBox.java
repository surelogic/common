...
public final class SearchBox {
  ...
  final Composite f_composite;

  public Composite getComposite() {
    return f_composite;
  }

  final Text f_searchText;

  public Text getTextControl() {
    return f_searchText;
  }

  final Label f_clearLabel;

  public Label getLabelControl() {
    return f_clearLabel;
  }

  final String f_graySearchText;

  final ISearchBoxObserver f_observer;

  final Image f_clearImage = SLImages.getImage(CommonImages.IMG_GRAY_X_LIGHT);

  final Image f_clearImageHover = SLImages.getImage(CommonImages.IMG_GRAY_X);

  boolean f_searchTextInUse = false;

  public SearchBox(final Composite parent, final String graySearchText, final String clearToolTipText,
      final ISearchBoxObserver observer) {
    if (parent == null)
      throw new IllegalArgumentException("parent must be non-null");
    if (graySearchText == null)
      throw new IllegalArgumentException("graySearchText must be non-null");
    if (clearToolTipText == null)
      throw new IllegalArgumentException("clearToolTipText must be non-null");
    if (observer == null)
    ...
    f_composite.setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT, true, false));
    ...
    /*
     * This label acts as a button via its low level SWT events. We do this to
     * conserve UI space and make the button appear more flat.
     */
    f_clearLabel = new Label(f_composite, SWT.NONE);
    f_clearLabel.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false, false));
    ...
    f_searchText.setForeground(f_searchText.getDisplay().getSystemColor(SWT.COLOR_GRAY));
    f_searchText.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
   ...
   * 
   * @param text
   *          the new search text. If the value is {@code null} or {@code ""}
   *          then the search text is cleared via a call to {@link #clearText()}
   *          .
   */
