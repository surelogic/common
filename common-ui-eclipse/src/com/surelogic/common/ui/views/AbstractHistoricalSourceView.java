...
public abstract class AbstractHistoricalSourceView extends ViewPart {
  ...
  public void createPartControl(final Composite parent) {
    ...
    f_sourceLabel.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));

    f_source = new StyledText(parent, SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER | SWT.READ_ONLY);
   ...
   *          unknown.
   */
  public final void setSourceSnapshotTime(final Date when) {
  ...
  private void populate(final ZipFile zf, final String path) throws IOException {
    ...
    /*
     * char[] buf = new char[4096]; int read; while ((read = br.read(buf)) >= 0)
  ...
  /**
   * Gets an interface to iterate through Zips of source code. Implementations
   * should not return {@code null} but rather an interation over an empty
   * collection.
   * 
   * @param run
   *          a string which names the run or scan.
   * @return an interface to iterate through the Zips of source code.
   */
  @NonNull
  ...
  protected static void tryToOpenInEditorUsingMethodName(final Class<?> viewClass, final String run, final String pkg,
  ...
  protected static void tryToOpenInEditorUsingFieldName(final Class<?> viewClass, final String run, final String pkg,
  ...
  /**
   * Shows the passed line number in the passed view and highlights the line via
   * selection.
   * 
   * @param view
  ...
  private static class MethodFinderVisitor extends TypeVisitor {
    ...
    public boolean visit(final MethodDeclaration node) {
      if (method.equals(node.getName().getIdentifier()) || method.equals("<init>") && node.isConstructor()) {
    ...
    public int getPosition() {
      return position != 0 ? position : possiblePosition != 0 ? possiblePosition : unlikelyPosition;
    }
  }
    ...
    public boolean visit(final VariableDeclarationFragment node) {
      if (inField && node.getName().getIdentifier().equals(field)) {
        ...
        if (currentType.equals(typeList)) {
          ...
          // Probably not our match, but if we don't find any
          // other
          // matching fields we will use it as a best guess.
          unlikelyPosition = nodePosition;
        }
  ...
  static class ASTVisitorRecorder extends ASTVisitor {
    ...
    public void preVisit(final ASTNode node) {
      ...
      b.append(ASTNode.nodeClassForType(node.getNodeType()).getSimpleName());
