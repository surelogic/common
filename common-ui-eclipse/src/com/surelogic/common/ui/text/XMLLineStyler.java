...
public class XMLLineStyler implements LineStyleListener 
{
	...
	public XMLLineStyler()
	{
		...
		colorContainer= new Color[] {
			...
			new Color(display, new RGB(0, 0, 128)),	
	...
	public void lineGetStyle(LineStyleEvent event) 
	{
		...
		while (token != EOL)
		{
			...
			StyleRange style = new StyleRange(xmlScanner.getStartOffsetOfToken() + event.lineOffset, xmlScanner.getTokenLength(), tokenColor, null);
			...
			}while (token != EOL && startOffset >=0 && endOffset < event.lineText.length() && event.lineText.substring(startOffset, endOffset).trim().length() <= 0);
		...
		{	
			...
			while (currentChar != EOL)
			{
				if (fallsUnderCommentBlock(lineStartOffset + tokenStartOffset))
				{
					...
					if (tokenLength - 3 >= 0 && lineText.substring (tokenStartOffset + tokenLength - 3, tokenStartOffset + tokenLength).equals("-->"))
				...
				{	
					...
					case '<':
						...
						/* Comment */
						else if (lineText.substring(tokenStartOffset).trim().length() >= 4 && lineText.substring(tokenStartOffset).trim().substring(0, 4).equals("<!--"))
						{
							...
							if (!(tokenStartOffset + tokenLength - 2 > tokenStartOffset + 3 && lineText.substring (tokenStartOffset + tokenLength - 3, tokenStartOffset + tokenLength).equals("-->")))
...
//				if ((startLineOffset <= commentBlock.start && commentBlock.start <= endLineOffset))
//					commentBlocks.remove(commentBlock);
//				else if (startLineOffset <= commentBlock.end && commentBlock.end >= endLineOffset)
//					commentBlocks.remove(commentBlock);					
//			}		
//		}
