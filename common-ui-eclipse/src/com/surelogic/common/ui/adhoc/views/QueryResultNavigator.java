 ...
 */
public final class QueryResultNavigator extends AdHocManagerAdapter implements ILifecycle {

  /**
   * Provides a new instance. {@link #init()} should be invoked on the instance.
   * {@link #dispose()} should be later invoked on the instance when the using
   * view is itself disposed.
   * 
   * @param source
  ...
  private AdHocQueryResult getForwardTarget() {
    ...
    if (selected != null) {
      if (selected.hasChildren()) {
        final AdHocQueryResult firstChild = selected.getChildrenList().get(0);
        return firstChild;
      }
    }
    return null;
  ...
  private final Action f_showDefinedVariablesAction = new Action() {
    ...
    public void run() {
      final AdHocQueryResult selected = getSelectedResult();
      if (selected != null) {
        final Map<String, String> variableValues = selected.getQueryFullyBound().getVariableValues();
  ...
  private void initActions() {
    ...
    f_backward.setToolTipText(I18N.msg("adhoc.query.navigator.backward.tooltip"));
    f_backward.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_LEFT));
    f_forward.setText(I18N.msg("adhoc.query.navigator.forward"));
    f_forward.setToolTipText(I18N.msg("adhoc.query.navigator.forward.tooltip"));
    f_forward.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_RIGHT));
    f_disposeAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_GRAY_X));
    f_disposeAction.setText(I18N.msg("adhoc.query.navigator.remove"));
    f_disposeAction.setToolTipText(I18N.msg("adhoc.query.navigator.remove.tooltip"));
    addEnableWhenAResultIsSelected(f_disposeAction);
    f_disposeAllAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_GRAY_X_DOUBLE));
    f_disposeAllAction.setText(I18N.msg("adhoc.query.navigator.removeAll"));
    f_disposeAllAction.setToolTipText(I18N.msg("adhoc.query.navigator.removeAll.tooltip"));
    addEnableWhenAResultExists(f_disposeAllAction);
    f_clearSelectionAction.setText(I18N.msg("adhoc.query.navigator.clearSelection"));
    f_clearSelectionAction.setToolTipText(I18N.msg("adhoc.query.navigator.clearSelection.tooltip"));
    f_clearSelectionAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_QUERY_BACK));
    addEnableWhenAResultIsSelected(f_clearSelectionAction);
    f_showDefinedVariablesAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_INFO));
    f_showDefinedVariablesAction.setText(I18N.msg("adhoc.query.navigator.showVariables"));
    f_showDefinedVariablesAction.setToolTipText(I18N.msg("adhoc.query.navigator.showVariables.tooltip"));
    addEnableWhenAResultIsSelected(f_showDefinedVariablesAction);
    f_showSqlAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_FILE));
    f_showSqlAction.setText(I18N.msg("adhoc.query.navigator.showSql"));
    f_showSqlAction.setToolTipText(I18N.msg("adhoc.query.navigator.showSql.tooltip"));
    addEnableWhenAResultIsSelected(f_showSqlAction);
    f_exportAction.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_EXPORT));
    f_exportAction.setText(I18N.msg("adhoc.query.navigator.exportData"));
    f_exportAction.setToolTipText(I18N.msg("adhoc.query.navigator.exportData.tooltip"));
  ...
  private void updateControlState() {
    ...
    if (aResultIsSelected) {
      ...
      if (hasData) {
        isTree = !((AdHocQueryResultSqlData) result).getModel().isPureTable();
      }
    }
