...
public final class QueryEditorMediator extends AdHocManagerAdapter implements
    ...
    QueryEditorMediator(AbstractQueryEditorView view, SashForm sash,
            Composite lhs, TabFolder lhsFolder, Table queryList,
            Tree queryTree, Button filterTreeCheck, Menu queryActionMenu,
            ToolItem runQuery, ToolItem newQuery, ToolItem deleteQuery,
            ToolItem saveQueries, PageBook rhs, Label noSelectionPane,
            Composite selectionPane, Text descriptionText, Text idText,
            Text cdText, Spinner sortHint, Combo type, Button showCheck,
            Button showAtRootCheck, TabFolder sqlFolder, StyledText sql,
    ...
    @Override
    public void init() {
        f_sash.setWeights(new int[] {
                EclipseUtility
                        .getIntPreference(CommonCorePreferencesUtility.QEDITOR_SASH_LHS_WEIGHT),
                EclipseUtility
                        .getIntPreference(CommonCorePreferencesUtility.QEDITOR_SASH_RHS_WEIGHT) });
        ...
        f_lhs.addListener(SWT.Resize, new Listener() {
            ...
            public void handleEvent(final Event event) {
                ...
                if (weights != null && weights.length == 2) {
                    EclipseUtility
                            .setIntPreference(
                                    CommonCorePreferencesUtility.QEDITOR_SASH_LHS_WEIGHT,
                                    weights[0]);
                    EclipseUtility
                            .setIntPreference(
                                    CommonCorePreferencesUtility.QEDITOR_SASH_RHS_WEIGHT,
                                    weights[1]);
                }
            }
        });
        ...
        f_lhsFolder
                .setSelection(EclipseUtility
                        .getIntPreference(CommonCorePreferencesUtility.QEDITOR_LHS_TAB_SELECTION));
        f_lhsFolder.addListener(SWT.Selection, new Listener() {
            @Override
            public void handleEvent(final Event event) {
                EclipseUtility.setIntPreference(
                        CommonCorePreferencesUtility.QEDITOR_LHS_TAB_SELECTION,
        ...
        f_sqlFolder
                .setSelection(EclipseUtility
                        .getIntPreference(CommonCorePreferencesUtility.QEDITOR_SQL_TAB_SELECTION));
        f_sqlFolder.addListener(SWT.Selection, new Listener() {
            @Override
            public void handleEvent(final Event event) {
                EclipseUtility.setIntPreference(
                        CommonCorePreferencesUtility.QEDITOR_SQL_TAB_SELECTION,
                        f_lhsFolder.getSelectionIndex());
            }
        });
        ...
        f_filterTree = EclipseUtility
                .getBooleanPreference(CommonCorePreferencesUtility.QEDITOR_FILTER_TREE_CHECK);
                ...
                if (filterTree != f_filterTree) {
                    ...
                    EclipseUtility
                            .setBooleanPreference(
                                    CommonCorePreferencesUtility.QEDITOR_FILTER_TREE_CHECK,
                                    filterTree);
        ...
        /*
         * When we are typing in the SQL editor, about every five seconds we
         * save what we are typing so that it gets shown in the Querydoc view.
         * This can be very helpful if you are editing the Querydoc for a query.
         */
        f_sql.addFocusListener(new FocusAdapter() {
            ...
            public void focusGained(FocusEvent e) {
                f_ts = new SWTTimingSource(5, TimeUnit.SECONDS, f_sql
                        .getDisplay());
                f_ts.addTickListener(new TimingSource.TickListener() {
                    @Override
                    public void timingSourceTick(TimingSource source,
                            long nanoTime) {
    ...
    private Image getImageForQuery(AdHocQuery query) {
        return SLImages.getImageForAdHocQuery(query.getType(),
                query.showAtRootOfQueryMenu(), !query.showInQueryMenu());
    }
}
