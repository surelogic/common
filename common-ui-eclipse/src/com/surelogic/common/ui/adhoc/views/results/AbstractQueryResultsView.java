...
public abstract class AbstractQueryResultsView extends ViewPart {
        ...
        final Action f_collapseAllAction = new Action() {
            ...
            public void run() {
                if (f_result != null && f_parent != null) {
                    for (final Control comp : ((Composite) f_parent
                            .getChildren()[0]).getChildren()) {
                        if (comp instanceof Composite) {
                            for (final Control c : ((Composite) comp)
                                    .getChildren()) {
    ...
    private Composite setupResultsPane(final AdHocQueryResult result) {
        ...
        if (XUtil.useExperimental
                || result.getManager().getDataSource().getQueryEditorViewId() != null
            ...
            queryDescription.setText(result.getQueryFullyBound().getQuery()
                    .getDescription());
        }
    ...
    private void displayResultSqlData(final AdHocQueryResultSqlData data) {
        ...
        if (data.getQueryFullyBound().getQuery().usesCustomDisplay()) {
             ...
             * CUSTOM
             */
            final String className = data.getQueryFullyBound().getQuery()
                    .getCustomDisplayClassName();
            try {
                final IQueryResultCustomDisplay customDisplay = (IQueryResultCustomDisplay) data
                        .getQueryFullyBound().getManager().getDataSource()
                        .getCustomDisplay(className);
                ...
                final String errorMsg = I18N.err(305, className, data
            ...
            boolean allowColumnSorting = data.getQueryFullyBound().getQuery()
                    .getMetaWithName(AdHocQuery.META_DISABLE_COLUMN_SORT) == null;
                ...
                for (int colI = 0; colI < columnCount; colI++) {
                    if (model.isColumnVisible(colI)) {
                        ...
                        if (!cell.getBlankText()) {
                            ...
                            Image image = SLImages
                                    .getImageForEncodedAdHocQueryResult(cell
            ...
            // minimize the column widths -- but with enough room if tree is
            // expanded
    ...
    private void addTreeCell(final TreeCell cell, final Tree tree,
        ...
        if (cell instanceof NonLeafTreeCell) {
            ...
            Image image = SLImages.getImageForEncodedAdHocQueryResult(nonLeaf
                    .getImageSymbolicName());
            if (image == null || !image.isDisposed()) {
                item.setImage(image);
            }
            for (final NonLeafColumnSummaryCell columnSummary : nonLeaf
                    .getColumnSummaries()) {
                ...
                if (itemIndex >= 0) {
                    if (!columnSummary.getBlankText()) {
                        ...
                        item.setForeground(itemIndex,
                                EclipseColorUtility.getSubtleTextColor());
                    }
                }
            }
            ...
            if (model.isPureTree()) {
                ...
                item.setImage(SLImages.getImageForEncodedAdHocQueryResult(leaf
                        .getImageSymbolicName()));
            } else {
                int itemIndex = 0;
                for (int colI = model.getLastTreeIndex(); colI < model
                    ...
                    if (model.isColumnVisible(colI)) {
                        ...
                        if (!rowCell.getBlankText()) {
                            ...
                            item.setImage(itemIndex, SLImages
                                    .getImageForEncodedAdHocQueryResult(rowCell
                                            .getImageSymbolicName()));
                        }
                        itemIndex++;
                    }
     ...
     */
    public static void setupSubQueryMenu(final Menu menu,
            final AdHocQueryResult queryResult,
        ...
        menu.addListener(SWT.Show, new Listener() {
            ...
            public void handleEvent(final Event event) {
                ...
                final AdHocQuery resultQuery = result.getQueryFullyBound()
                        .getQuery();
                final List<AdHocSubQuery> subQueries = resultQuery
                        .getVisibleSubQueryList();
                final Map<String, String> variableValues = result
                        .getVariableValues();
                final Map<String, String> topVariableValues = result
                        .getTopVariableValues();
                if (extraVariables != null) {
                    /*
                     * Only for custom result displays that need to add
                     * variables not expressed by selecting a row of the query
                     * data.
                     */
                    ...
                    public void handleEvent(final Event event) {
                        if (event.widget.getData() instanceof AdHocQuery) {
                            ...
                            final AdHocQueryFullyBound boundQuery = new AdHocQueryFullyBound(
                                    query, variableValues, topVariableValues);
                ...
                @Nullable
                final AdHocSubQuery defaultSubQuery = AdHocSubQuery.sortHelper(
                        subQueries, variableValues, queryResult
                                .getQueryFullyBound().getQuery());

                for (final AdHocSubQuery subQuery : subQueries) {
                    final AdHocQuery query = subQuery.getQuery();
                    final boolean isCompletelySubstituted = query
                    ...
                    if (isCompletelySubstituted || showUnrunnableQueries) {
                        ...
                        item.setData(query);
                        final boolean decorateAsDefault = subQuery
                                .equals(defaultSubQuery);
                        item.setImage(SLImages.getImageForAdHocQuery(
                                query.getType(), decorateAsDefault, false));
    ...
    public static void copySelection(final AdHocQueryResult queryResult) {
        if (queryResult != null
                && queryResult instanceof AdHocQueryResultSqlData) {
            final AdHocQueryResultSqlData result = (AdHocQueryResultSqlData) queryResult;
            ...
            if (selected != -1) {
                ...
                if (selectedC != null) {
                    ...
                    for (final NonLeafColumnSummaryCell nlcs : selectedC
                            .getColumnSummaries()) {
    ...
    /**
     * Used to run the default sub query for a result. If no such query exists
     * or the query result does not contain data nothing occurs.
     * <p>
     * May be used by custom query result displays.
     *
     * @param queryResult
     ...
     *            {@code null} except for custom query result displays.
     */
    public static void runDefaultQueryOf(final AdHocQueryResult queryResult,
            @Nullable final Map<String, String> extraVariables) {
        if (queryResult != null
                && queryResult instanceof AdHocQueryResultSqlData) {
            ...
            final Map<String, String> topVariableValues = data
                    .getTopVariableValues();
            final ArrayList<AdHocSubQuery> subQueries = data
                    .getQueryFullyBound().getQuery().getVisibleSubQueryList();
            @Nullable
            final AdHocSubQuery defaultSubQuery = AdHocSubQuery.sortHelper(
                    subQueries, variableValues, data.getQueryFullyBound()
                            .getQuery());
            if (defaultSubQuery != null) {
                final AdHocQueryFullyBound boundQuery = new AdHocQueryFullyBound(
                        defaultSubQuery.getQuery(), variableValues,
                        topVariableValues);
