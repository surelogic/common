...
public class PromisesAnnotationRewriter {
  ...
  static final String PROMISE_PKG = "com.surelogic";
  final ASTParser parser;
  ASTNode ast;
  ASTRewrite rewrite;
  final Set<String> allowsMultipleAnnos;

  /**
   * @param annos
   *          A set of annotation names that allow multiple annotations on a
   *          given declaration
   */
  ...
  public void writeAnnotations(final Set<AnnotationDescription> descs, final TextEditGroup editGroup) {
  ...
  private class AnnotationVisitor extends ASTVisitor {
    ...
    private void checkForDeclsToCreate(AbstractTypeDeclaration node, IDeclType type) {
      ...
      final List<BodyDeclaration> toAdd = new ArrayList<>();
      final List<AnnotationDescription> scopedPromises = new ArrayList<>();
    ...
    private void handleParameters(final MethodDeclaration m) {
      ...
      for (Object o : m.parameters()) {
        ...
        rewriteNode(p, SingleVariableDeclaration.MODIFIERS2_PROPERTY, findTargets(inParameter), inParameter);
        i++;
      }
    }
     ...
     * @param list
     */
    private void rewriteNode(final ASTNode node, final ChildListPropertyDescriptor prop, final List<AnnotationDescription> list,
        final IDecl target) {
      if (list != null && !list.isEmpty()) {
        Collections.sort(list);
        Collections.reverse(list);
        final List<List<AnnotationDescription>> anns = new ArrayList<>();
     ...
     * @param target
     */
    private void mergeAnnotations(final AST ast, final List<AnnotationDescription> ann, final ListRewrite lrw, final IDecl target) {
      ...
      for (final ASTNode aNode : nodes) {
        if (aNode instanceof Annotation) {
          ...
          if (m.match(a)) {
            lrw.replace(aNode, m.merge(ast, a, target, imports), editGroup);
            return;
          }
        }
      }
    ...
    public boolean visit(final FieldDeclaration node) {
      ...
      final List<AnnotationDescription> list = new ArrayList<>();
      IDeclField f = null;
    ...
    public void endVisit(final CompilationUnit node) {
      ...
      if (imports.size() > 0) {
        ...
        final ListRewrite lrw = rewrite.getListRewrite(node, CompilationUnit.IMPORTS_PROPERTY);
  ...
  abstract class AbstractMergeable implements Mergeable {
    protected void extractExistingAnnos(final Annotation a, final Set<AnnoSummary> contents) {
      if (a.isNormalAnnotation()) {
        ...
        final Map<String, String> attrs = new HashMap<>(4);
        String value = null;
    ...
    private void handlePossibleArray(Annotation a, Set<AnnoSummary> contents, Expression e) {
      if (e instanceof ArrayInitializer) {
        ...
        handleExistingAnno(new AnnoSummary(a.getTypeName().toString(), extractValue(e), Collections.<String, String> emptyMap()),
            contents);
      }
    }
    ...
    String extractValue(Expression e) {
      ...
      /*
       * if (e instanceof StringLiteral) { final String lit = ((StringLiteral)
       * e).getLiteralValue(); return lit; } else if (e instanceof
       * InfixExpression) { final InfixExpression ex = (InfixExpression) e; if
       * (ex.getOperator() == Operator.PLUS) { // Assume to be a string
       * concatenation StringBuilder sb = new StringBuilder();
       * sb.append(extractString(ex.getLeftOperand()));
       * sb.append(extractString(ex.getRightOperand())); if
       * (ex.hasExtendedOperands()) { for(Object o : ex.extendedOperands()) {
       * sb.append(extractString((Expression) o)); } } } } throw new
  ...
  class AssumptionMergeStrategy extends AbstractMergeable {
    ...
    AssumptionMergeStrategy(final List<AnnotationDescription> anns) {
      this.newAnnotations = new ArrayList<>(anns);
    }

    @Override
    public Annotation merge(final AST ast, final Annotation cur, final IDecl target, final Set<String> imports) {
      final Set<AnnoSummary> existing = new HashSet<>();
      if (cur != null) {
        extractExistingAnnos(cur, existing);
      }
      for (final AnnotationDescription desc : newAnnotations) {
        addImport(ASSUME, imports);
        String value = String.format("%s for %s", desc.toString(), ScopedPromiseUtil.getForSyntax(desc.getTarget()));
        existing.add(new AnnoSummary(ASSUME, value, Collections.<String, String> emptyMap()));
      }
      final List<AnnoSummary> sortedAnns = new ArrayList<>(existing);
      Collections.sort(sortedAnns);
      final List<Annotation> anns = new ArrayList<>(sortedAnns.size());
    ...
    public boolean match(final Annotation a) {
      final String aName = a.getTypeName().getFullyQualifiedName().replaceAll(".*\\.", "");
  ...
  class CommaDelimitedMergeStrategy extends AbstractMergeable {
    ...
    CommaDelimitedMergeStrategy(final String name, final List<AnnotationDescription> anns) {
    ...
    @Override
    public Annotation merge(final AST ast, final Annotation a, final IDecl target, final Set<String> imports) {
      addImport(name, imports);
      final TreeSet<AnnoSummary> contents = new TreeSet<>();
      for (final AnnotationDescription desc : newAnnotations) {
  ...
  /**
   * Adds the annotation, wrapping the whole thing in a 'plural' annotation if
   * needed
   */
  class DefaultMergeStrategy extends AbstractMergeable {
    ...
    DefaultMergeStrategy(final List<AnnotationDescription> anns) {
      ...
      if (altA.isEmpty()) {
        ...
        altAnnotations = new HashMap<>(altA.size());
    ...
    @Override
    public Annotation merge(final AST ast, final Annotation cur, final IDecl target, final Set<String> imports) {
      final Set<AnnoSummary> newContents = new HashSet<>();
      for (final AnnotationDescription desc : newAnnotations) {
      ...
      return createWrappedAnnotation(ast, name, wrapper, newContents, imports);
    }
  ...
  interface Mergeable {
     ...
     * @param imports
     *          the set of imports used. This should be updated as . necessary.
     * @return
     */
    Annotation merge(AST ast, Annotation a, IDecl target, Set<String> imports);

  }

  Annotation createWrappedAnnotation(final AST ast, final String name, final String wrapperName, final Set<AnnoSummary> contents,
    ...
    if (len == 1) {
      ...
      final List<AnnoSummary> cs = new ArrayList<>(contents);
      Collections.sort(cs);
  ...
  Annotation createWrappedAssume(final AST ast, final List<Annotation> anns, final Set<String> imports) {
  ...
  private Annotation ann(final AST ast, final String name, final AnnoSummary anno, final Set<String> imports) {
    addImport(name, imports);
    switch (anno.getKind()) {
    case Normal:
  ...
  static class AnnoSummary implements Comparable<AnnoSummary> {
    ...
    public boolean equals(final Object obj) {
      ...
      return AnnotationDescription.isSame(annotation, other.annotation) && AnnotationDescription.isSame(contents, other.contents)
