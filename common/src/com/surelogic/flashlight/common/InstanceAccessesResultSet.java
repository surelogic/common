...
public class InstanceAccessesResultSet implements InvocationHandler {
  ...
  InstanceAccessesResultSet(ResultSet set) throws SQLException {
    this.set = set;
    if (set.next()) {
  ...
  public static ResultSet create(Connection conn, long receiverId, Timestamp begin, Timestamp end) throws SQLException {
    PreparedStatement st = conn.prepareStatement(QB.get("Accesses.prep.selectAccessesInBlock"));
  ...
  public static ResultSet create(Connection conn, long receiverId, Timestamp begin, Timestamp end, long fieldId, long secondFieldId)
      throws SQLException {
    PreparedStatement st = conn.prepareStatement(QB.get("Accesses.prep.selectAccessesInBlockByFields"));
    st.setLong(1, receiverId);
    st.setTimestamp(2, begin);
    st.setTimestamp(3, end);
