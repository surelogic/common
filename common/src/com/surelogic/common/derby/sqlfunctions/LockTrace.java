...
public class LockTrace {
  ...
  LockTrace(long lockTraceId, long parentId, long lockId, long traceId, LockType type, String packageName, String className,
  ...
  public static DBQuery<LinkedList<LockTrace>> lockTrace(final long traceId) {
    return new DBQuery<LinkedList<LockTrace>>() {
      ...
      public LinkedList<LockTrace> perform(final Query q) {
        final Queryable<LockTrace> getTrace = q.prepared("LockTrace.selectById", SingleRowHandler.from(new LockTraceRowHandler()));
        final LinkedList<LockTrace> traces = new LinkedList<>();
        if (traceId != -1) {
  ...
  static class LockTraceRowHandler implements RowHandler<LockTrace> {
    ...
    public LockTrace handle(Row r) {
      return new LockTrace(r.nextLong(), r.nextLong(), r.nextLong(), r.nextLong(), LockType.fromFlag(r.nextString()),
          r.nextString(), r.nextString(), r.nextString(), r.nextString(), r.nextString(), r.nextInt());
    }
