...
public abstract class DerbyConnection extends AbstractDBConnection {
  ...
  /**
   * Sets, for use upon IDE restart, if the embedded database should be deleted.
   *
   * @param value
   *          {@code true} indicates the embedded database should be deleted
   *          when the IDE restarts, {@code false} indicates that it should not.
   */
  ...
  protected final String getConnectionURL() {
    return JDBC_PRE + getDatabaseLocation() + (getSchemaName() == null ? "" : JDBC_POST + getSchemaName());
  }
        ...
        try {
          if (dbDir.exists()) {
            if (FileUtility.recursiveDelete(dbDir)) {
              SLLogger.getLogger().info("Database deleted at startup : " + getDatabaseLocation());
            } else {
              SLLogger.getLogger().log(Level.SEVERE, "Unable to delete database at startup : " + getDatabaseLocation());
            }
          }
        } finally {
   ...
   *
   * @return {@code true} if the embedded database has been booted and checked,
  ...
  public Connection getConnection() throws SQLException {
    final Connection conn = LazyPreparedStatementConnection.wrap(DriverManager.getConnection(getConnectionURL()));
    return conn;
  }
  ...
  public synchronized void shutdown() {
    if (f_booted) {
      try {
        synchronized (this) {
          DriverManager.getConnection(getConnectionURL() + JDBC_SHUTDOWN);
        }
        throw new IllegalStateException("The database at " + getConnectionURL() + " did not shut down properly.");
      } catch (final SQLException e) {
        if (e.getErrorCode() == 45000) {
          SLLogger.getLogger().log(Level.FINE, e.getMessage());
        } else {
          throw new IllegalStateException("The database at " + getConnectionURL() + " did not shut down properly.", e);
        }
      }
      f_booted = false;
    }
