...
public interface IJavaFactory<P extends ISLJavaProject> {
  ...
  static final IJavaFactory<JavaProject> prototype = new IJavaFactory<JavaProject>() {
    public JavaProject newProject(JavaProjectSet<JavaProject> projects, Config config, String name, SLProgressMonitor monitor) {
      return new JavaProject(projects, config, name, monitor);
    }

    public JavaProjectSet<JavaProject> newProjectSet(File loc, boolean isAuto, Date time, Map<String, Object> args) {
      return new JavaProjectSet<>(this, loc, isAuto, time, args);
    }
  };
}
