...
public class JavaClassPath<PS extends JavaProjectSet<?>> implements IJavacClassParser {
	...
	public final void map(String destProj, IJavaFile file) {
		...
		if (!classToFile.containsKey(key)) {
			/*
			 * if (!key.first().startsWith(Config.JRE_NAME) && file.getFile() !=
			 * null) { System.out.println("Mapping "+key.second()+" to "
			 * +file.getFile()); }
			 */
			/*
			 * System.out.println("Mapping "+destProj+" : "
			 * +file.getQualifiedName()); if
			 * ("com.surelogic.common.ref.Decl".equals(file.getQualifiedName()))
			 * { System.out.println(); }
			 */
