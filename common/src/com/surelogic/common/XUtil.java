...
public final class XUtil {
	...
	private static final String f_updateScript = System
			.getProperty("SureLogicUpdateScript");
	static {
	...
	public static final boolean testingWorkspace = System.getProperty(
			"jsure.test.workspace", "false").equals("true");
	...
	private static String computeRecordScriptValue() {
		...
		if (f_updateScript != null) {
			if (f_updateScript.endsWith(".zip")) {
				...
				if (temp != null) {
					System.out
							.println("Overriding SureLogicRecordScript: using "
									+ prefix + ", instead of " + temp);
				}
				return prefix;
			} else {
				throw new IllegalStateException("Not ending with .zip: "
						+ f_updateScript);
			}
		}
		return temp;
	 ...
	 * 
	 * @return The (relative) directory to which the script (and other
