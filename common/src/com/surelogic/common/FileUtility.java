...
public final class FileUtility {
	 ...
	 * filesystem. Clients should check the results on the filesystem if the
	 * empty directories could cause problems
	 * 
	 * @param withinPath
	 *            the path to delete empty directories within. This path will
	 *            not be deleted even if it ends up being empty after this
	 *            method executes.
	 * 
	...
	private static boolean deleteEmptySubDirectoriesHelper(final File file) {
		...
		if (file.isDirectory()) {
			...
			if (elements.length == 0) {
				...
				/*
				 * Note that often remaining files are reported (at least on
				 * Windows) and we have to check if everything in the list
	...
	public static String getStreamContentsAsString(String label,
			InputStream input) {
		try {
	...
	/**
	 * Iterate over all the files in the specified directory, using the
	 * filerunner as a FileFilter
	 */
	public static void recursiveIterate(final FileRunner r, File f) {
		recursiveIterate(r, null, f);
	}

	private static void recursiveIterate(final FileRunner r,
			final String relativePath, File f) {
		if (f.isFile()) {
			if (f.length() != 0) {
