...
public class SLLogger {
  ...
  static {
    ...
    if (property != null) {
      ...
      try {
        ...
        /*
         * 
         * We can't log this problem normally as we are setting up logging.
         */
        System.err.println(I18N.err(99, SL_LOGGING_PROPERTY, property));
      }
      LEVEL = new AtomicReference<>(value);
    } else {
  ...
  /**
   * Changes the logging level of the console and file handlers managed by this
   * class. Calling this method causes the <code>SLLoggingLevel</code> property
   * to be ignored.
   * <p>
   ...
   */
  @UniqueInRegion("LoggerState")
  private static final AtomicReference<SLFormatter> f_formatter = new AtomicReference<>(new SLFormatter());

  /**
   * A simple cache of loggers we have already configured.
   */
  @UniqueInRegion("LoggerState")
  private static final Map<String, Logger> f_nameToLogger = new HashMap<>();

  /**
   * A list of the handlers we manage for logging.
   */
  @UniqueInRegion("LoggerState")
  private static final List<Handler> f_handlers = new ArrayList<>();
  ...
  static {
     ...
     * container. The EJB container, due to re-deployments, can load this class
     * many times. We use a System property to avoid doing this SureLogic
     * logging setup more than a single time.
     */
   ...
   * send logging output to its parent's handlers. It will be registered in the
  ...
  public static synchronized Logger getLogger(final String name) {
    ...
    final String loggerName = "com.surelogic" + ("".equals(name) ? "" : "." + name);
