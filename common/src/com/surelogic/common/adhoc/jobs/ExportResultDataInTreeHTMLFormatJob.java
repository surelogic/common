...
public class ExportResultDataInTreeHTMLFormatJob extends ExportResultDataInHTMLFormatJob {

  private final boolean f_hasTable;
  private String[] f_headers;
  private final String[] f_treeState;

  public ExportResultDataInTreeHTMLFormatJob(final AdHocQueryResultSqlData data, final File file) {
    super(data, file);
    final int lastTreeIndex = data.getModel().getLastTreeIndex();
    f_treeState = lastTreeIndex == -1 ? SLUtility.EMPTY_STRING_ARRAY : new String[data.getModel().getLastTreeIndex()];
  ...
  protected void writeHtmlFooter(final PrintWriter writer) {
    ...
    FileUtility.copy(cl.getResource("/com/surelogic/common/adhoc/jobs/outline.js"), new File(f_parentDir, "outline.js"));
  }
