...
public final class AdHocQueryResultSqlData extends AdHocQueryResult {
	...
	/*
	 * Only one of the below two can be selected at a time so the other should
	 * be -1 or null.
	 * 
	...
	private final int f_rowsSkipped;
	private final String[] f_accessKeys;

	public boolean isRowLimited() {
		return f_rowsSkipped > 0;
	}

	public int getNumRowsOmitted() {
		return f_rowsSkipped;
	}
	...
	/**
	 * Gets the row index selected in this result or -1 if nothing is selected
	...
	public void setSelectedRowIndex(final int rowIndex) {
		if (rowIndex < -1 || rowIndex >= f_model.getRowCount()) {
			throw new IllegalArgumentException(I18N.err(127, rowIndex, f_model
					.getRowCount()));
		}
	...
	/**
	 * Gets the selected partial result row or {@code null} if nothing is
	 ...
	 *         created result as well as any variable values defined by
	 *         selection of a row.
	 */
	public Map<String, String> getVariableValues() {
		 ...
		 */
		final Map<String, String> result = getQueryFullyBound()
				.getVariableValues();
		if (f_selectedRowIndex != -1) {
			...
			/*
			 * Add in all the variable values from the selected partial result
			 * row.
			 */
	...
	public AdHocQueryResultSqlData(final AdHocManager manager,
			final AdHocQueryResultSqlData parent,
			final AdHocQueryFullyBound query,
			final ResultSetUtility.Result results, final DBConnection datasource)
			throws Exception {
		super(manager, parent, query, datasource);

		final AdornedTreeTableModel model = AdornedTreeTableModel.getInstance(
				results.columnLabels, results.rows);
		if (model == null) {
		...
		f_model = model;
		f_rowsSkipped = results.rowsSkipped;
		f_accessKeys = parent.getAccessKeys();
	}

	public AdHocQueryResultSqlData(final AdHocManager manager,
			final AdHocQueryFullyBound query,
			final ResultSetUtility.Result results,
			final DBConnection datasource, final String... accessKeys)
			throws Exception {
