 ...
 * <li>The character {@link #SEP}.
 * <li>The string constant {@link #MACADDRESS_LABEL} followed by each string in
 * the set returned by <tt>licenseNetCheck.getMacAddresses()</tt>. The set may
 * be empty.
 * <li>The character {@link #SEP} (between each MAC address).
 * </ul>
 ...
 * id=58eab27c-e416-4b76-9225-49783707056f
 * date=2010-06-11
 * macAddress=60-a4-4c-61-20-40
 * macAddress=08-00-27-00-68-fb
 * </pre>
 * 
...
public final class SLLicensePersistence {
  ...
  public static String toString(final SLLicenseNetCheck licenseNetCheck) {
    ...
    final StringBuilder b = new StringBuilder();
    b.append(UUID_LABEL).append(licenseNetCheck.getUuid().toString()).append(SEP);
    b.append(DATE_LABEL).append(SLUtility.toStringDay(licenseNetCheck.getDate())).append(SEP);
    for (String macAddress : licenseNetCheck.getMacAddresses())
      b.append(MACADDRESS_LABEL).append(macAddress).append(SEP);
    return b.toString();
  }

  /**
   * Converts a string representation to a license. The string representation
   * should have been created by a call to {@link #toString(SLLicense)}. If the
  ...
  public static SLLicense toLicense(final String value) {
    ...
    if (uuidS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, UUID_LABEL, value));
      return null;
    }
    final UUID uuid;
    try {
      ...
      SLLogger.getLogger().log(Level.WARNING, I18N.err(188, uuidS, value), e);
      return null;
    }
    ...
    if (productS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, PRODUCT_LABEL, value));
      return null;
    }
    final SLLicenseProduct product = SLLicenseProduct.fromString(productS);
    if (product == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(199, productS, value));
      return null;
    }

    /*
     * Parse license license duration in days from installation until expiration
     * or renewal.
     */
    final String durationInDaysS = getValue(value, DURATION_LABEL);
    if (durationInDaysS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, DURATION_LABEL, value));
      return null;
    }
    ...
    try {
      ...
      SLLogger.getLogger().log(Level.WARNING, I18N.err(191, durationInDaysS, value), e);
      return null;
    }

    /*
     * Parse license installation deadline. This value is optional so it may not
     * exist.
     */
    ...
    if (dateS == null) {
      ...
      try {
        ...
        SLLogger.getLogger().log(Level.WARNING, I18N.err(189, dateS, value), e);
        return null;
      }
    }
    ...
    if (typeS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, TYPE_LABEL, value));
      return null;
    }
    final SLLicenseType type = SLLicenseType.fromString(typeS);
    if (type == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(190, typeS, value));
      return null;
    }
    ...
    if (maxActiveS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, MAXACTIVE_LABEL, value));
      return null;
    }
    final int maxActive;
    try {
      ...
      SLLogger.getLogger().log(Level.WARNING, I18N.err(191, maxActiveS, value), e);
      return null;
    }
    ...
    if (performNetCheckS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, PERFORMNETCHECK_LABEL, value));
      return null;
    }
    /*
     * Here we enable net checks if the string is not strictly, ignoring case,
     * "false". The call Boolean.getBoolean does the opposite, which would err
     * toward a false value, disabling net checks, and not a true one.
     */
    final boolean performNetCheck = !("false".equalsIgnoreCase(performNetCheckS));
   ...
   * {@code null} is returned and a message about what is wrong with the string
   * is logged.
   * 
   * @param value
  ...
  public static SLLicenseNetCheck toLicenseNetCheck(final String value) {
    ...
    if (dateS == null) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(187, DATE_LABEL, value));
      return null;
    }
    final Date date;
    try {
    ...
    try {
      result = new SLLicenseNetCheck(uuid, date, macAddresses);
    } catch (Exception e) {
   ...
   *          signature.
   * @param middleMark
   *          a known string to separate the digital signature from the data.
   * @param endMark
   *          a known string to mark the end of the data.
   * @param linewrap
   *          a width (in characters) to line wrap the returned string, a value
   *          less than 1 indicates no wrapping is to be performed.
   * @return a digitally signed encoded hex string representation (perhaps line
   *         wrapped) of <tt>data</tt>
   */
  public static String toSignedHexString(final byte[] data, final PrivateKey key, final String beginMark, final String middleMark,
      final String endMark, final int linewrap) {
   ...
   *         <tt>license</tt>.
   */
  public static String toSignedHexString(final SLLicense license, final PrivateKey key, final boolean linewrap) {
    final byte[] data = toString(license).getBytes();
    return toSignedHexString(data, key, BEGIN_LICENSE, MIDDLE, END_LICENSE, linewrap ? LINEWRAP : 0);
  }
   ...
   */
  public static String toSignedHexString(final SLLicenseNetCheck licenseNetCheck, final PrivateKey key, final boolean linewrap) {
    final byte[] data = toString(licenseNetCheck).getBytes();
    return toSignedHexString(data, key, BEGIN_NET_CHECK, MIDDLE, END_NET_CHECK, linewrap ? LINEWRAP : 0);
  }
   ...
   * @param licenses
   *          the list of {@link PossiblyActivatedSLLicense} objects to convert.
   * 
   * @see SLLicensePersistence#toSignedHexString(List)
   */
  public static void writeLicensesToFile(File newFile, final List<PossiblyActivatedSLLicense> licenses) {
    if (newFile == null)
   ...
   */
  public static void writeLicenseToFile(File newFile, PossiblyActivatedSLLicense license) {
    if (newFile == null)
      throw new IllegalArgumentException(I18N.err(44, "newFile"));
    if (license == null)
      throw new IllegalArgumentException(I18N.err(44, "license"));
   ...
   *          {@code true} if the returned string should be line wrapped,
   *          {@code false} for no line wrapping.
   * @return a string that contains a set of licenses and license net checks as
   *         digitally signed encoded hex strings.
   */
  public static String toSignedHexString(final List<PossiblyActivatedSLLicense> licenses, final boolean linewrap) {
    if (licenses == null)
      throw new IllegalArgumentException(I18N.err(44, "licenses"));
   ...
   * null} results if the string is not well-formed or its is improperly signed.
   * <p>
   ...
   * @param endMark
   *          a known string to mark the end of the data.
   * @return the data, or {@code null} if <tt>s</tt> is not well-formed or it is
   *         improperly signed.
   */
  public static byte[] getData(final String s, final PublicKey key, final String beginMark, final String middleMark,
    ...
    /*
     * Trim off any extra characters before or after the signed license data.
     */
    ...
    /*
     * Check that the data is properly signed. This checks, with some assurance
     * that SureLogic produced the data.
     */
   ...
   * @param key
   *          a RSA public key used to check the digital signature.
   * @return a license object or {@code null} if <tt>s</tt> is not well-formed,
   *         improperly signed, or missing required license data.
   */
   ...
   * well-formed, improperly signed, or missing required license net check data.
   * <p>
   * If the conversion fails (and {@code null} is returned) then diagnostic
   * information is written to the log about what went wrong.
   * 
   ...
   *          a RSA public key used to check the digital signature.
   * @return a license object or {@code null} if <tt>s</tt> is not well-formed,
   *         improperly signed, or missing required license net check data.
   */
  public static SLLicenseNetCheck toLicenseNetCheck(final String s, final PublicKey key) {
    final byte[] data = getData(s, key, BEGIN_NET_CHECK, MIDDLE, END_NET_CHECK);
    if (data == null) {
      return null;
    } else {
   ...
   */
  public static List<PossiblyActivatedSLLicense> readLicensesFromFile(final File textFile) {
    if (textFile == null)
      throw new IllegalArgumentException(I18N.err(44, "textFile"));
    try {
      final String fileContents = FileUtility.getFileContentsAsString(textFile);
      return readPossiblyActivatedLicensesFromString(fileContents);
    } catch (Exception trouble) {
      SLLogger.getLogger().log(Level.WARNING, I18N.err(206, textFile.getAbsolutePath()), trouble);
    }
   ...
   *         objects.
   */
  public static List<PossiblyActivatedSLLicense> readPossiblyActivatedLicensesFromString(final String s) {
    if (s == null)
    ...
    for (SignedSLLicenseNetCheck licenseNetCheck : licenseNetChecks) {
      final SignedSLLicense associatedLicense = lookupLicense(licenses, licenseNetCheck.getLicenseNetCheck().getUuid());
      if (associatedLicense == null) {
        SLLogger.getLogger().log(Level.WARNING, I18N.err(200, licenseNetCheck.getLicenseNetCheck().getUuid(), s));
      } else {
        /*
         * Create the paired license and license net check and add it to our
         * result.
         */
        result.add(new PossiblyActivatedSLLicense(associatedLicense, licenseNetCheck));
        /*
         * Ensure that we remove the associated license from the remaining
         * unmatched licenses.
         */
  ...
  /**
   * Reads a string that contains a set of licenses as digitally signed encoded
   * hex strings and returns a list of {@link SignedSLLicense} objects.
   * 
   * @param s
   *          the string.
  ...
  public static List<SignedSLLicense> readLicensesFromString(final String s) {
    ...
    while (true) {
      ...
      final SignedSLLicense license = SignedSLLicense.getInstance(b.substring(index, endIndex));
      result.add(license);
      fromIndex = endIndex;
    }
    return result;
   ...
   *         objects.
   */
  public static List<SignedSLLicenseNetCheck> readLicenseNetChecksFromString(final String s) {
    if (s == null)
      throw new IllegalArgumentException(I18N.err(44, "s"));
    ...
    while (true) {
      ...
      final SignedSLLicenseNetCheck licenseNetCheck = SignedSLLicenseNetCheck.getInstance(b.substring(index, endIndex));
  ...
  /**
   * Helper method to find a {@link SignedSLLicense} in a list with a particular
   * {@link UUID}.
   * 
   * @param licenses
   ...
   *         {@link UUID}.
   */
  private static SignedSLLicense lookupLicense(List<SignedSLLicense> licenses, UUID id) {
    for (SignedSLLicense license : licenses) {
   ...
   */
  public static SLLicenseNetCheck inputLicenseNetCheckFromSignedFile(File in, PublicKey key) {
