...
public final class SignedSLLicenseNetCheck {
  ...
  private SignedSLLicenseNetCheck(final SLLicenseNetCheck licenseNetCheck, final String signedHexString) {
  ...
  /**
   * Creates a signed license net check object from a passed hex encoded string
   * using the public key obtained from {@link SLUtility#getPublicKey()} to
   * check that the strings digital signature.
   * 
   * @param signedHexString
   *          a signed hex string that demonstrates that the license net check
   *          was created by SureLogic.
   * @return a signed license net check object.
   * @throws IllegalArgumentException
   *           if any parameter to this method is {@code null}.
   */
  public static SignedSLLicenseNetCheck getInstance(@NonNull String signedHexString) {
    return getInstance(signedHexString, SLUtility.getPublicKey());
  }

  /**
   * Creates a signed license net check object from a passed hex encoded string
   * using a public key to check that the strings digital signature.
   * 
   * @param signedHexString
   *          a signed hex string that demonstrates that the license net check
   *          was created by the private key corresponding to <tt>key</tt>.
   * @param key
   *          a public key.
   * @return a signed license net check object.
   * @throws IllegalArgumentException
   *           if any parameter to this method is {@code null}.
   */
  public static SignedSLLicenseNetCheck getInstance(@NonNull String signedHexString, @NonNull PublicKey key) {
    ...
    SLLicenseNetCheck nc = SLLicensePersistence.toLicenseNetCheck(signedHexString, key);
    if (nc == null)
   ...
   *           if any parameter to this method is {@code null}.
   */
  public static SignedSLLicenseNetCheck getInstance(@NonNull SLLicenseNetCheck licenseNetCheck, @NonNull PrivateKey key) {
    if (licenseNetCheck == null)
      throw new IllegalArgumentException(I18N.err(44, "licenseNetCheck"));
    if (key == null)
      throw new IllegalArgumentException(I18N.err(44, "key"));
    final String signedHexString = SLLicensePersistence.toSignedHexString(licenseNetCheck, key, false);
