...
public abstract class AbstractRemoteSLJob {
  ...
  protected final void run() {
    ...
    /*
     * System.out.println("port = "+port); if (true) { throw new
    ...
    final TestCode testCode = TestCode.getTestCode(System.getProperty(RemoteSLJobConstants.TEST_CODE_PROPERTY));
    ...
    try {
      ...
      switch (testCode) {
      case SCAN_FAILED:
        outputFailure(out, "Testing scan failure", new Throwable());
        break;
      case ABNORMAL_EXIT:
        System.exit(-RemoteSLJobConstants.ERROR_PROCESS_FAILED);
        break;
  ...
  /**
   * Do any setup before running the job e.g. reading system properties to
   * initialize state
   * 
   * @return The initialized job to run
   */
  protected abstract SLJob init(BufferedReader br, Monitor mon) throws Throwable;
  ...
  void outputFailure(final PrintStream out, final String msg, final Throwable e) {
    final StackTraceElement[] trace = e.getStackTrace();
    /*
     * out.println("Caught exception"); for (final StackTraceElement ste :
     * trace) { out.println("\t at " + ste); }
     */
    if (msg == null) {
  ...
  protected void checkInput(final BufferedReader br, final Monitor mon, final String msg) throws IOException {
    out.println(msg);
    checkIfCancelled(br, mon);
    out.flush();
  }

  void checkIfCancelled(final BufferedReader br, final Monitor mon) throws IOException {
    if (br.ready()) {
  ...
  protected class Monitor implements SLProgressMonitor {
    ...
    Monitor(BufferedReader br, final PrintStream out) {
      this.br = br;
      this.out = out;
    }
    ...
    @Override
    public void begin() {
      throw new IllegalStateException("begin() can't be used in this context");
    }
  ...
  protected class LogHandler extends Handler {
    ...
    public void publish(LogRecord record) {
      ...
      final String message = record.getMessage() == null ? "" : record.getMessage();
      final StringBuilder b = new StringBuilder();
      SLFormatter.formatMsgTail(b, message, record.getSourceClassName(), record.getSourceMethodName());
