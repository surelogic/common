...
public final class ConfigHelper {
    ...
    public void addPluginToPath(final String pluginId, boolean unpacked) {
        ...
        if (unpacked) {
            ...
            if (loc.isDirectory()) {
                ...
                // workspace
                if (!workspaceExists) {
                    boolean jarExists = addToPath(
                            pluginDir + "/" + loc.getName() + ".jar", false); // in
                    // Ant
                    if (!jarExists) {
     ...
     * @return true if found
     */
    public boolean addPluginJarsToPath(final String pluginId,
            boolean exclusive, String... jars) {
        boolean rv = true;
        String pluginDir = getPluginDir(pluginId);
        if (pluginDir != null) {
            File pluginFile = new File(pluginDir);
            if (pluginFile.exists() && !pluginFile.isDirectory()
                    && pluginDir.endsWith(".jar")) {
                // When we have a jar, we look for a directory that matches the
                // jar (minus the suffix)
                pluginDir = pluginDir.substring(0, pluginDir.length() - 4);
            }
        }
        for (String jar : jars) {
            boolean exists = addToPath(pluginDir + '/' + jar, !exclusive);
            if (exclusive && exists) {
                return true;
            }
            rv = rv && exists;
    ...
    public boolean addToPath(File f, boolean required) {
        ...
        if (!exists) {
            if (required) {
                // FIX
                throw new RuntimeException("Missing required library: "
                        + f.getAbsolutePath());
                /*
    ...
    protected void findJars(File folder) {
        if (!folder.exists()) {
            SLLogger.getLogger().warning(
                    "Unable to find jars in non-existent folder: " + folder);
        }
