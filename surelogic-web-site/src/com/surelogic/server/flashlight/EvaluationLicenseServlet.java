...
public class EvaluationLicenseServlet extends HttpServlet {
  ...
  private final Logger log = SLLogger.getLoggerFor(EvaluationLicenseServlet.class);
   ...
   */
  private boolean generateAndEmailLicense(final String firstName, final String lastName, final String company, final String toEmail) {
    // Generate the license file
    try {
      final File lic = File.createTempFile("flashlight", ".lic");
      try {

        final SLLicense license = new SLLicense(UUID.randomUUID(), toEmail, SLLicenseProduct.FLASHLIGHT, EVALUATION_DURATION,
            new Date(System.currentTimeMillis() + INSTALL_BEFORE_PERIOD), SLLicenseType.USE, 1, true);
        ...
        ServicesDBConnection.getInstance().withTransaction(new NullDBQuery() {
          ...
          public void doPerform(final Query q) {
            q.prepared("WebServices.insertEvaluationLicense").call(new Date(), toEmail, firstName, lastName, company, lic);
          }
        });
        return true;
