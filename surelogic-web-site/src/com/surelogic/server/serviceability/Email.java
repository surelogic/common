...
public class Email {
	 ...
	 */
	static void sendEmail(final String subject, final String content,
			final String to, final boolean sendBCC) {
				...
				try {
					...
					if (to == null) {
						msg.setRecipient(Message.RecipientType.TO,
								new InternetAddress(config.getTo()));
					} else {
						msg.setRecipient(Message.RecipientType.TO,
					...
					final StringBuilder msg = new StringBuilder(
							"Error emailing support request to: ");
					if (to != null) {
						msg.append(to);
						if (sendBCC) {
	...
	static final class EmailConfig {
		...
		public EmailConfig(final Properties javaMailProperties,
				final String from, final String to, final Properties bugProps) {
			super();
