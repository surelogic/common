...
public class LicenseSearchServlet extends HttpServlet {
	...
	private void handle(final HttpServletRequest req,
			final HttpServletResponse resp) throws ServletException,
			IOException {
		ServicesDBConnection.getInstance().withReadOnly(
				new SearchQuery(req, resp));
	}
	...
	private static class SearchQuery extends HTMLQuery {
		...
		public SearchQuery(final HttpServletRequest req,
		...
		public void doPerform(final Query q) {
			...
			writer
					.println("<h3><a href=\"admin\">To License Overview</a></h3>");
			writer
					.println("<h3><a href=\"log\">To Recent License Activity</a></h3>");
			...
			writer
					.println(String
							.format(
									...
									search == null ? "" : search));
			tableBegin();
			tableRow(DATE.th("Latest Activity"), STRING.th("License"), STRING
					.th("Holder"), STRING.th("Product"), NUMBER.th("Installs"),
					NUMBER.th("Renewals"), NUMBER.th("Removals"), NUMBER
							.th("Blacklists"), NUMBER.th("Too Many Installs"));
			NullRowHandler handler = new NullRowHandler() {
				@Override
				protected void doHandle(final Row r) {
					Date latest = r.nextTimestamp();
					String uuid = r.nextString();
					SLLicenseProduct p = SLLicenseProduct.fromString(r
							.nextString());
					...
					tableRow(DATE.td(latest), STRING.td(uuid(uuid)), STRING
							.td(holder), STRING.td(p.toString()), NUMBER
							.td(installs), NUMBER.td(renewals), NUMBER
							.td(removals), NUMBER.td(blacklisted), NUMBER
							.td(tooMany));
				}
			};
