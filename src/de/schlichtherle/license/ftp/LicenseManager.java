...
public class LicenseManager extends de.schlichtherle.license.LicenseManager {
	...
	/* => "ftpKeyStoreParam" */
	private static final String FTP_KEYSTORE_PARAM = SLUtility.toString(
			new long[] { 0x5AC618555EC967B9L, 0xABE6B36EE9502E44L,
					0x58814EE1E5E3CF8FL }).toString();

	/* => "Equal key store parameters for regular and FTP licenses!" */
	private static final String EQUAL_KEY_STORE_PARAMS = SLUtility
			.toString(new long[] { 0x4C72769A851F67C3L, 0x40A67DFB8206B61BL,
					0x412E3FE142BA378EL, 0x68B2E25FA4BE7607L,
					0x7A6AB29D2E2E5FDDL, 0x0432DC122188C63FL,
	...
	/* => "user" */
	private static final String CN_FTP_USER = CN
	 ...
	 * manager as if it had been newly created. The implementation of this class
	 * asserts that the given parameter object is an instance of
	 * {@link de.schlichtherle.license.ftp.LicenseParam} and applies some
	 * plausibility checks to it to ensure that it adheres to the contract of
	 ...
	 *             the current policy.
	 */
	public synchronized void setLicenseParam(
			de.schlichtherle.license.LicenseParam param)
		...
		final KeyStoreParam ftpKeyStoreParam = licenseParam
				.getFTPKeyStoreParam();
		...
		if (0 >= ftpDays || ftpDays > 365) // unreasonable
			throw new IllegalArgumentException(
					"unreasonable trial period < 0 or > 365 days");
	...
	protected synchronized LicenseContent verify(final LicenseNotary notary)
		...
		try {
			...
			content.setNotAfter(ftpNotAfter(param.getFTPDays())); // enforce
			// expire
			// date
			content = install(create(content, ftpNotary), ftpNotary); // reassign
			// because
			// of
			// clone
	...
	/**
	 * Returns a license notary configured to use the free trial period keystore
	 * parameters contained in the current license parameters - <tt>null</tt> is
	 * never returned.
	 */
	protected synchronized LicenseNotary getFTPLicenseNotary() throws Exception {
		if (ftpNotary == null)
			ftpNotary = new LicenseNotary(((LicenseParam) getLicenseParam())
					.getFTPKeyStoreParam());
