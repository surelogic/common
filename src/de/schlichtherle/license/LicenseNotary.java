...
public class LicenseNotary {
	...
	/* => "param" */
	static final String PARAM = SLUtility.toString(new long[] {
			0x9462FFDE0183752L, 0xE2A34A222A24DB14L });

	/* => "alias" */
	private static final String ALIAS = SLUtility.toString(new long[] {
			0xF7122BB1103EE24L, 0x5D073BF77D50CE8AL });

	/* => "exc.noKeyPwd" */
	private static final String EXC_NO_KEY_PWD = SLUtility.toString(new long[] {
			0x9BEEC1A930D89BC1L, 0x314F8BFE96B1D7BL, 0x7D41D459E6191D0AL });

	/* => "exc.noKeyEntry" */
	private static final String EXC_NO_KEY_ENTRY = SLUtility
			.toString(new long[] { 0xECC3EE809CC45994L, 0x395EC0314F8227A1L,
					0x90B1DBA3D701F0FBL });

	/* => "exc.privateKeyOrPwdIsNotAllowed" */
	private static final String EXC_PRIVATE_KEY_OR_PWD_IS_NOT_ALLOWED = SLUtility
			.toString(new long[] { 0xD6E9FE0BD39F8075L, 0x351D278C14FABB1AL,
					0xD64A9C9BD412AB10L, 0x1AEB0F657DB66448L,
					0x41EE587D2CD73A1AL });

	/* => "exc.noCertificateEntry" */
	private static final String EXC_NO_CERTIFICATE_ENTRY = SLUtility
			.toString(new long[] { 0xCA437064C1D0C41EL, 0xDDBBA0FF1F17FC35L,
					0x5D2CD0D970444C3DL, 0xF94EAAC3F634D04CL });

	/* => "SHA1withDSA" */
	private static final String SHA1_WITH_DSA = SLUtility.toString(new long[] {
			0xEB0CFFD676FD2839L, 0x176DF514D5A0ED59L, 0xBFE1DE24AEF8E9B0L });

	/* => "JKS" */
	private static final String JKS = SLUtility.toString(new long[] {
			0xA97AF8FB6356CB08L, 0x20E47C2995D2FE7AL });

	private KeyStoreParam param; // init by setKeyStoreParam() - should be
	// accessed via getKeyStoreParam() only!
	 ...
	 * 
	 * @param param
	 *            The keyStore configuration parameters - may <em>not</em> be
	 *            <tt>null</tt>.
	 * 
	...
	/**
	 * Sets the keyStore configuration parameters. Calling this method resets
	 * the notary as if it had just been created.
	 * 
	 * @param param
	 *            The keyStore configuration parameters - may <em>not</em> be
	 *            <tt>null</tt>.
	...
	/**
	 * Encodes and signs the given <tt>content</tt> and returns a locked generic
	 * certificate holding the encoded content and its digital signature.
	 * <p>
	 * Please note the following:
	 * <ul>
	 * <li>Because this method locks the certificate, a subsequent call to
	 * {@link #sign(GenericCertificate, Object)} or
	 * {@link #verify(GenericCertificate)} is redundant and will throw a
	 * <tt>PropertyVetoException</tt>. Use {@link GenericCertificate#isLocked()}
	 * to detect whether a generic certificate has been successfuly signed or
	 * verified before or call {@link GenericCertificate#getContent()} and
	 * expect an Exception to be thrown if it hasn't.</li>
	 * <li>There is no way to unlock the returned certificate. Call the copy
	 * constructor of {@link GenericCertificate} if you need an unlocked copy of
	 * the certificate.</li>
	 * </ul>
	 * 
	 * @param content
	 *            The object to sign. This must either be a JavaBean or an
	 *            instance of any other class which is supported by <tt>
	 *            {@link de.schlichtherle.xml.PersistenceService}</tt> - maybe
	 *            <tt>null</tt>.
	 * 
	 * @return A locked generic certificate holding the encoded content and its
	 *         digital signature.
	 * 
	 * @throws Exception
	 *             A subclass of this class may be thrown for various reasons.
	 */
	 ...
	 * certificate is already locked, i.e. if it has been signed or verified
	 * before.</li>
	 * <li>Because this method locks the certificate, a subsequent call to
	 * {@link #sign(GenericCertificate, Object)} or
	 * {@link #verify(GenericCertificate)} is redundant and will throw a
	 ...
	 * verified before or call {@link GenericCertificate#getContent()} and
	 * expect an Exception to be thrown if it hasn't.</li>
	 * <li>There is no way to unlock the certificate. Call the copy constructor
	 * of {@link GenericCertificate} if you need an unlocked copy of the
	 * certificate.</li>
	 * </ul>
	 * <p>
	 * This method exists for testing purposes only and you should not normally
	 * need it. Use {@link #sign(Object)} instead.
	 * 
	 * @param certificate
	 *            The generic certificate used to hold the encoded content and
	 ...
	 *             A subclass of this class may be thrown for various reasons.
	 */
	public/* synchronized */void sign(GenericCertificate certificate,
	 ...
	 * 
	 * @param certificate
	 *            The generic certificate to verify - may <em>not</em> be
	 *            <tt>null</tt>.
	 * 
	 * @throws Exception
	 *             A subclass of this class may be thrown for various reasons.
	 ...
	 *             If the parameters used to access the corresponding key store
	 *             are insufficient or incorrect. Note that you should always
	 *             use {@link Throwable#getLocalizedMessage()} to get a
	 *             (possibly localized) meaningful detail message.
	 * @throws IOException
	 ...
	 *             If the key cannot get recovered (e.g. the given password is
	 *             wrong).
	 */
	protected/* synchronized */PrivateKey getPrivateKey()
			throws LicenseNotaryException, IOException, CertificateException,
		...
		if (privateKey == null) {
			...
			try {
				privateKey = (PrivateKey) keystore.getKey(alias, keyPwd
						.toCharArray());
	...
	protected/* synchronized */PublicKey getPublicKey()
		...
		if (publicKey == null) {
			...
			final KeyStore keystore = getKeyStore();
			try {
				if ((getKeyStoreParam().getKeyPwd() != null) != keystore
						.isKeyEntry(alias))
	 ...
	 * {@link GenericCertificate} - <tt>null</tt> is never returned.
	 * <p>
	 * <b>Experimental:</b> Methods marked with this note have been tested to be
	 * functional but may change or disappear at will in one of the next
	 * releases because they are still a topic for research on extended
	 ...
	 *             cannot be found.
	 */
	protected/* synchronized */KeyStore getKeyStore() throws IOException,
