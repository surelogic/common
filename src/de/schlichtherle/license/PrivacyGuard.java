...
public class PrivacyGuard {
	...
	/* => "PBEWithMD5AndDES" */
	private static final String PBE_WITH_MD5_AND_DES = SLUtility
			.toString(new long[] { 0x27B2E8783E47F1ABL, 0x45CF8AD4390DC9D8L,
					0xAB320350966BC9BFL });

	private CipherParam param; // initialized by setCipherParam() - should be
								// accessed via getCipherParam() only!
	 ...
	 * valid and cannot be used unless {@link #setCipherParam(CipherParam)} is
	 * called!
	 */
	 ...
	 * 
	 * @param param
	 *            The cipher configuration parameters - may <em>not</em> be
	 *            <tt>null</tt>.
	 */
	...
	/**
	 * Sets the cipher configuration parameters. Calling this method resets the
	 * guard as if it had been newly created. Some plausibility checks are
	...
	/**
	 * Encodes, compresses and encrypts the given license certificate and
	 * returns the result as a license key. Please note that this method does
	 * not sign the certificate.
	 * 
	 * @param certificate
	 *            The license certificate - may <em>not</em> be <tt>null</tt>.
	 * 
	...
	public/* synchronized */byte[] cert2key(final GenericCertificate certificate)
		...
		final OutputStream out = new GZIPOutputStream(new CipherOutputStream(
				keyOut, getCipher4Encryption()));
		try {
	...
	/**
	 * Decrypts, decompresses and decodes the given license key and returns the
	 * result as a license certificate. Please note that this method does not
	 * verify the certificate.
	 * 
	 * @param key
	 *            The license key to process - may <em>not</em> be <tt>null</tt>
	 *            .
	 * 
	 * @return The license certificate - <tt>null</tt> is never returned.
	 * 
	 * @throws Exception
	 *             An instance of a subclass of this class for various reasons.
	 *             Note that you should always use
	...
	public/* synchronized */GenericCertificate key2cert(final byte[] key)
			throws Exception {
		final InputStream in = new GZIPInputStream(new ByteArrayInputStream(
	 ...
	 * decryption - <tt>null</tt> is never returned.
	 * <p>
	 * <b>Experimental:</b> Methods marked with this note have been tested to be
	 * functional but may change or disappear at will in one of the next
	 * releases because they are still a topic for research on extended
	...
	protected/* synchronized */Cipher getCipher() {
		...
		algoParamSpec = new PBEParameterSpec(new byte[] { (byte) 0xce,
				(byte) 0xfb, (byte) 0xde, (byte) 0xac, (byte) 0x05,
				(byte) 0x02, (byte) 0x19, (byte) 0x71 }, 2005);

		try {
			KeySpec keySpec = new PBEKeySpec(getCipherParam().getKeyPwd()
					.toCharArray());
