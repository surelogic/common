 ...
 * @since 3.4
 */
public abstract class AbstractInformationControl implements
...
//		IInformationControlExtension5 {
{
	 ...
	 */
	public AbstractInformationControl(final Shell parentShell,
	...
	public AbstractInformationControl(final Shell parentShell,
		...
		this(parentShell, SWT.TOOL | SWT.ON_TOP | SWT.RESIZE, null,
				toolBarManager);
	}
	...
	public AbstractInformationControl(final Shell parentShell,
		...
		this(parentShell, SWT.TOOL | SWT.ON_TOP
				| (isResizable ? SWT.RESIZE : 0), null, null);
	}
	 ...
	 */
	@Deprecated
	AbstractInformationControl(final Shell parentShell, int shellStyle,
			final String statusFieldText, final ToolBarManager toolBarManager) {
		...
		final Color foreground = display
				.getSystemColor(SWT.COLOR_INFO_FOREGROUND);
		final Color background = display
		...
		fContentComposite.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true));
		fContentComposite.setLayout(new FillLayout());
		setColor(fContentComposite, foreground, background);

		createStatusComposite(statusFieldText, toolBarManager, foreground,
				background);
	}

	private void createStatusComposite(final String statusFieldText,
			final ToolBarManager toolBarManager, final Color foreground,
		...
		final GridData gridData = new GridData(SWT.FILL, SWT.BOTTOM, true,
				false);
		fStatusComposite.setLayoutData(gridData);
		final GridLayout statusLayout = new GridLayout(1, false);
	...
	private void createStatusLabel(final String statusFieldText,
		...
		final FontData[] fontDatas = JFaceResources.getDialogFont()
				.getFontData();
		...
		fStatusLabel.setForeground(fStatusLabel.getDisplay().getSystemColor(
				SWT.COLOR_WIDGET_DARK_SHADOW));
		fStatusLabel.setBackground(background);
		setColor(fStatusComposite, foreground, background);
	}

	private void createToolBar(final ToolBarManager toolBarManager) {
		...
		final Composite spacer = new Composite(bars, SWT.NONE);
	...
	private void addResizeSupportIfNecessary(final Composite bars) {
		...
		final int size = getResizeHandleSize(bars);

		final GridData data = new GridData(SWT.END, SWT.END, false, true);
		data.widthHint = size;
		data.heightHint = size;
		resizer.setLayoutData(data);
		resizer.addPaintListener(new PaintListener() {
			public void paintControl(final PaintEvent e) {
				final Point s = resizer.getSize();
				final int x = s.x - 2;
				final int y = s.y - 2;
				final int min = Math.min(x, y);
				if (isWin) {
					// draw dots
					e.gc.setBackground(resizer.getDisplay().getSystemColor(
					...
					// draw diagonal lines
					e.gc.setForeground(resizer.getDisplay().getSystemColor(
							SWT.COLOR_WIDGET_NORMAL_SHADOW));
		...
		resizer.setCursor(resizer.getDisplay().getSystemCursor(
				isRTL ? SWT.CURSOR_SIZESW : SWT.CURSOR_SIZESE));
		final MouseAdapter resizeSupport = new MouseAdapter() {
			private MouseMoveListener fResizeListener;

			@Override
			public void mouseDown(final MouseEvent e) {
				final Rectangle shellBounds = fShell.getBounds();
				...
				final Point mouseLoc = resizer.toDisplay(e.x, e.y);
				final int mouseX = mouseLoc.x;
				final int mouseY = mouseLoc.y;
				fResizeListener = new MouseMoveListener() {
					public void mouseMove(final MouseEvent e2) {
						final Point mouseLoc2 = resizer.toDisplay(e2.x, e2.y);
						final int dx = mouseLoc2.x - mouseX;
						final int dy = mouseLoc2.y - mouseY;
						if (isRTL) {
	...
	private int getResizeHandleSize(final Composite parent) {
	...
	private void addMoveSupport(final Control control) {
		final MouseAdapter moveSupport = new MouseAdapter() {
			private MouseMoveListener fMoveListener;

			@Override
			public void mouseDown(final MouseEvent e) {
				final Point shellLoc = fShell.getLocation();
				final int shellX = shellLoc.x;
				final int shellY = shellLoc.y;
				final Point mouseLoc = control.toDisplay(e.x, e.y);
				final int mouseX = mouseLoc.x;
				final int mouseY = mouseLoc.y;
				fMoveListener = new MouseMoveListener() {
					public void mouseMove(final MouseEvent e2) {
						final Point mouseLoc2 = control.toDisplay(e2.x, e2.y);
						final int dx = mouseLoc2.x - mouseX;
						final int dy = mouseLoc2.y - mouseY;
	 ...
	 */
	private static void setColor(final Control control, final Color foreground,
			final Color background) {
	...
	protected final Point getSizeConstraints() {
		return fSizeConstraints != null ? Geometry.copy(fSizeConstraints)
				: null;
	}
	...
	public Rectangle computeTrim() {
		...
		if (fStatusComposite != null) {
			trim.height += fStatusComposite.computeSize(SWT.DEFAULT,
					SWT.DEFAULT).y;
		}
	...
	public void setFocus() {
		final boolean focusTaken = fShell.setFocus();
		if (!focusTaken) {
			fShell.forceFocus();
		}
	}
			...
			fShellListener = new Listener() {
				...
				public void handleEvent(final Event event) {
					...
					for (int i = 0; i < listeners.length; i++) {
						final FocusListener focusListener = (FocusListener) listeners[i];
	 ...
	 */
	public void setStatusText(final String statusFieldText) {
