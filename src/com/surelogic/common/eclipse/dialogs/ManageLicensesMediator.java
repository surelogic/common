...
final class ManageLicensesMediator implements ILifecycle {
	...
	ManageLicensesMediator(Table licenseTable, Button installFromFileButton,
			Button installFromClipboardButton, Button activateButton,
	...
	private void updateTableContents() {
		...
		for (final PossiblyActivatedSLLicense installedLicense : SLLicenseManager
				.getInstance().getLicenses()) {
			final TableItem item = new TableItem(f_licenseTable, SWT.NONE);
			final SLLicense license = installedLicense.getSignedSLLicense()
					.getLicense();
			item.setText(0, license.getProduct().toString());
			setLicenseImage(item, installedLicense);
			final String activationExpl = installedLicense
					.getActivatedExplanation();
	...
	private void setLicenseImage(final TableItem item,
			final PossiblyActivatedSLLicense installedLicense) {
		if (installedLicense.isActivated()) {
			if (installedLicense.isExpired())
				item.setImage(0,
						SLImages.getImage(CommonImages.IMG_LICENSE_GRAY));
			else
			...
			if (installedLicense.isPastInstallBeforeDate())
				item.setImage(0, SLImages
						.getImage(CommonImages.IMG_LICENSE_NO_SEAL_GRAY));
			else
				item.setImage(0,
						SLImages.getImage(CommonImages.IMG_LICENSE_NO_SEAL));
		}
	}

	private List<PossiblyActivatedSLLicense> getLicenseTableSelection() {
	 ...
	 */
	private void installLicenseFromFile() {
		FileDialog fileDialog = new FileDialog(getShell(), SWT.OPEN);
		final String title = I18N
				.msg("common.manage.licenses.dialog.install.file.dialog.title");
		fileDialog.setText(title);
		fileDialog.setFilterPath(System.getProperty("user.home"));
		final String path = fileDialog.open();
		if (path != null) {
			try {
				final String value = FileUtility
						.getFileContentsAsString(new File(path));
				SLLicenseUtility.tryToInstallLicense(value);
			} catch (Exception e) {
				final int code = 141;
				final String msg = I18N.err(
						code,
						path,
						e.getMessage() == null ? e.getClass().getName() : e
								.getMessage());
				final SLStatus status = SLStatus
						.createErrorStatus(code, msg, e);
				final String errorDialogTitle = I18N
						.msg("common.manage.licenses.dialog.install.failure");
				ErrorDialogUtility.open(getShell(), errorDialogTitle,
						SLEclipseStatusUtility.convert(status));
			}
			updateTableContents();
			updateButtonState();
		}
	...
	private void installLicenseFromClipboard() {
		 ...
		 */
		final String value = ShowTextDialog
				.mutateText(
						getShell(),
						I18N.msg("common.manage.licenses.dialog.installFromClipboard.dialog.title"),
						data);
		if (value == null)
		...
		try {
			...
			final int code = 145;
			final String msg = I18N.err(code, e.getMessage() == null ? e
					.getClass().getName() : e.getMessage(), value);
	 ...
	 */
	private void uninstallLicenses(List<PossiblyActivatedSLLicense> selection) {
		...
		String confirmMsg = I18N.msg(
				"common.manage.licenses.dialog.uninstall.msg",
				count > 1 ? count + " licenses" : "\""
						+ selection.get(0).getSignedSLLicense().getLicense()
								.getProduct().toString() + "\" license");
		if (anyActivatedWithANetCheck)
			confirmMsg = confirmMsg
					+ I18N.msg("common.manage.licenses.dialog.uninstall.netcheckwarn");
		if (!MessageDialog.openConfirm(getShell(),
				I18N.msg("common.manage.licenses.dialog.uninstall.title"),
				confirmMsg)) {
			return; // bail
		}
		try {
			...
			final SLStatus status = SLStatus.createErrorStatus(code, msg, e);
			final String errorDialogTitle = I18N
					.msg("common.manage.licenses.dialog.uninstall.failure");
			ErrorDialogUtility.open(getShell(), errorDialogTitle,
					SLEclipseStatusUtility.convert(status));
		}
		updateTableContents();
	 ...
	 * @param activation
	 *            {@true} if this action is a license activation, {@code} false
	 *            if this action is a license renewal.
	 */
	private void activateRenewLicenses(
			List<PossiblyActivatedSLLicense> selection, boolean activation) {
		final int count = selection.size();
		if (count < 1)
			return;
		...
		if (!MessageDialog.openConfirm(getShell(), I18N.msg(
				"common.manage.licenses.dialog.activaterenew.title", passive),
				I18N.msg("common.manage.licenses.dialog.activaterenew.msg",
						active, count > 1 ? count + " licenses" : "\""
								+ selection.get(0).getSignedSLLicense()
										.getLicense().getProduct().toString()
								+ "\" license"))) {
			return; // bail
		}
		...
		try {
			...
			final String msg = I18N.err(code, e.getMessage());
			final SLStatus status = SLStatus.createErrorStatus(code, msg, e);
			final String errorDialogTitle = I18N.msg(
					"common.manage.licenses.dialog.activaterenew.failure",
					active);
			ErrorDialogUtility.open(getShell(), errorDialogTitle,
					SLEclipseStatusUtility.convert(status));
		}
