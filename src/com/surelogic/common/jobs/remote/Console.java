...
public class Console extends Thread {	
	 ...
	 * connections).
	 */
	private final List<WeakReference<IClientHandler>> f_handlers = new ArrayList<WeakReference<IClientHandler>>();
	...
	public void run() {
		...
		if (!listening) {
			logAProblem("unable to listen on any port between " + f_port
					+ " and " + port
		...
		while (!f_shutdownRequested) {
			try {
				...
				log("console connect from "
						...
						+ handler.getName() + ")");
				final WeakReference<IClientHandler> p_handler = new WeakReference<IClientHandler>(
						handler);
				...
				logAProblem("failure listening for client connections "
						+ port, e);
			}
		}
		/*
	...
	void requestShutdown() {
		...
		try {
			...
			logAProblem("unable to close the socket used by "
					+ getName(), e);
		}
	}
