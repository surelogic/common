...
public final class ExportQueryDialog extends Dialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		directions.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false,
				false));
		...
		fileNameText.setText(System.getProperty("user.home")
				+ System.getProperty("file.separator") + "adhoc.xml");
		...
		f_mediator = new Mediator(exportAllToggle, queryGroup, queryTable,
				fileNameText, browseButton);
		f_mediator.init();
		return container;
	}
	...
	class Mediator {
		...
		Mediator(final Button exportAllToggle, final Group queryGroup,
				final Table queryTable, final Text fileNameText,
		...
		void doExport() {
			...
			if (f_exportAllToggle.getSelection()) {
				...
				AdHocPersistence.save(selectedQueries, exportFile, true);
			}
		}
