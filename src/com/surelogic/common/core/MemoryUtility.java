...
public class MemoryUtility {
	 ...
	 * half way between {@code minMb} and {@code maxMb} is a possible heap
	 * memory size.
	 * 
	 * @param minMb
	 ...
	 *            attempts to determine the maximum that can be supported on
	 *            this platform.
	...
	/**
	 * Runs a trivial Java process to see if it doesn't die because the
	 * specified heap size is too large.
	 * <p>
	 * Consider the example of passing 1024 to this method. It effectively runs
	 * <tt>java -Xmx1024m -version</tt> which either works if 1024 megabytes is
	 * a possible heap size on this platform or the job fails. The
	 * <tt>-version</tt> causes the version to be output and the command to exit
	 * immediately in the case that the heap size is possible on this platform.
	 * <p>
