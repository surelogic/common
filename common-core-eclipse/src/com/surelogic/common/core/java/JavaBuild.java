...
public class JavaBuild {
  ...
  protected static List<Pair<IResource, Integer>> pairUpResources(List<IResource> resources, Integer kind) {
    if (resources.isEmpty()) {
      return Collections.emptyList();
    }
    final List<Pair<IResource, Integer>> result = new ArrayList<>(resources.size());
    for (IResource r : resources) {
      result.add(new Pair<>(r, kind));
    }
    return result;
  }

  private static final Map<?, ?> buildArgs = Collections.singletonMap(BUILD_KIND,
      Integer.toString(IncrementalProjectBuilder.FULL_BUILD));

  public static <PS extends JavaProjectSet<P>, P extends ISLJavaProject> PS analyze(AbstractJavaScanner<PS, P> scanner,
      List<IJavaProject> selectedProjects, IErrorListener l) {
    ...
    try {
      ...
      try {
        for (IJavaProject p : JDTUtility.getAllRequiredProjects(selectedProjects)) {
          boolean noErrors = XUtil.testing || JDTUtility.noCompilationErrors(p, new NullProgressMonitor());
          if (!noErrors) {
            l.reportError("Compile Errors in " + p.getElementName(), "JSure is unable to analyze " + p.getElementName()
      ...
      for (IJavaProject p : selectedProjects) {
        ...
        scanner.registerBuild(p.getProject(), buildArgs, pairUpResources(jpr.resources, IResourceDelta.ADDED), jpr.cus);
      ...
      SLLogger.getLogger().log(Level.SEVERE, "Failure setting up to analyze: " + selectedProjects, e);
      return null;
    }
  }
}
