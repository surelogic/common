...
/**
 * Creates a new Java project with the specified source files, or an empty
...
public class CreateProject extends AbstractCommand {
  ...
  public boolean execute(ICommandContext context, String... contents) throws Exception {
    ...
    if (!p.exists()) {
      IProjectDescription description = p.getWorkspace().newProjectDescription(contents[1]);
       ...
       * 
       * // Add the JSure Nature if
       * (!description.hasNature(Nature.DOUBLE_CHECKER_NATURE_ID)) { String[]
       * natures = description.getNatureIds(); String[] newNatures = new
       * String[natures.length + 1]; System.arraycopy(natures, 0, newNatures, 0,
       * natures.length); newNatures[natures.length] =
       * Nature.DOUBLE_CHECKER_NATURE_ID; description.setNatureIds(newNatures);
       * }
       */
